...
;------GETING THE LOCK------
LOCK2_T0: LL $2, 128($0); load the lock (128 loc) to $2
BNE $0, $2, LOCK2_T0; check to see if it is locked already ? go back : keep going
ADDI $2, $2, 1; set the lock 
SC $2, 128($0); store the lock back to memory
BEQ $0, $2, LOCK2_T0; if failed, go back to the beginning
...
;------GETING THE LOCK------
LOCK2_T1: LL $2, 128($0); load the lock (128 loc) to $2
BNE $0, $2, LOCK2_T1; check to see if it is locked already ? go back : keep going
ADDI $2, $2, 1; set the lock 
SC $2, 128($0); store the lock back to memory
BEQ $0, $2, LOCK2_T1; if failed, go back to the beginning
;------END OF GETING THE LOCK------
...
;------GETING THE LOCK------
LOCK2_T2: LL $2, 128($0); load the lock (128 loc) to $2
BNE $0, $2, LOCK2_T2; check to see if it is locked already ? go back : keep going
ADDI $2, $2, 1; set the lock 
SC $2, 128($0); store the lock back to memory
BEQ $0, $2, LOCK2_T2; if failed, go back to the beginning
;------END OF GETING THE LOCK------
...
;------GETING THE LOCK------
LOCK2_T3: LL $2, 128($0); load the lock (128 loc) to $2
BNE $0, $2, LOCK2_T3; check to see if it is locked already ? go back : keep going
ADDI $2, $2, 1; set the lock 
SC $2, 128($0); store the lock back to memory
BEQ $0, $2, LOCK2_T3; if failed, go back to the beginning
;------END OF GETING THE LOCK------
