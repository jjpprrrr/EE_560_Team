...
;#################################################################
;------LOADING OPERANDS------
LW $3, 16($0);  loading A0 to $3
LW $4, 80($0); loading B0 to $4
MULT $5, $3, $4; multiply A0 with B0 and put the result in $5
;------END OF LOADING OPERANDS------

;------GETING THE LOCK------
LOCK1_T0: LL $2, 128($0); load the lock (128 loc) to $2
BNE $0, $2, LOCK1_T0; check to see if it is locked already ? go back : keep going
ADDI $2, $2, 1; set the lock 
SC $2, 128($0); store the lock back to memory
BEQ $0, $2, LOCK1_T0; if failed, go back to the beginning
...
;#################################################################
;------LOADING OPERANDS------
LW $3, 20($0);  loading A0 to $3
LW $4, 84($0); loading B0 to $4
MULT $5, $3, $4; multiply A0 with B0 and put the result in $5
;------END OF LOADING OPERANDS------

;------GETING THE LOCK------
LOCK1_T1: LL $2, 128($0); load the lock (128 loc) to $2
BNE $0, $2, LOCK1_T1; check to see if it is locked already ? go back : keep going
ADDI $2, $2, 1; set the lock 
SC $2, 128($0); store the lock back to memory
BEQ $0, $2, LOCK1_T1; if failed, go back to the beginning
;------END OF GETING THE LOCK------
...
;#################################################################
;------LOADING OPERANDS------
LW $3, 24($0);  loading A0 to $3
LW $4, 88($0); loading B0 to $4
MULT $5, $3, $4; multiply A0 with B0 and put the result in $5
;------END OF LOADING OPERANDS------

;------GETING THE LOCK------
LOCK1_T2: LL $2, 128($0); load the lock (128 loc) to $2
BNE $0, $2, LOCK1_T2; check to see if it is locked already ? go back : keep going
ADDI $2, $2, 1; set the lock 
SC $2, 128($0); store the lock back to memory
BEQ $0, $2, LOCK1_T2; if failed, go back to the beginning
;------END OF GETING THE LOCK------
...
;#################################################################
;------LOADING OPERANDS------
LW $3, 28($0);  loading A0 to $3
LW $4, 92($0); loading B0 to $4
MULT $5, $3, $4; multiply A0 with B0 and put the result in $5
;------END OF LOADING OPERANDS------

;------GETING THE LOCK------
LOCK1_T3: LL $2, 128($0); load the lock (128 loc) to $2
BNE $0, $2, LOCK1_T3; check to see if it is locked already ? go back : keep going
ADDI $2, $2, 1; set the lock 
SC $2, 128($0); store the lock back to memory
BEQ $0, $2, LOCK1_T3; if failed, go back to the beginning
;------END OF GETING THE LOCK------
