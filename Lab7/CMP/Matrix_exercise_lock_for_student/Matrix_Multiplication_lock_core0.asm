...
;#################################################################
;------LOADING OPERANDS------
LW $3, 0($0);  loading A0 to $3
LW $4, 64($0); loading B0 to $4
MULT $5, $3, $4; multiply A0 with B0 and put the result in $5
;------END OF LOADING OPERANDS------

;------GETING THE LOCK------
LOCK0_T0: LL $2, 128($0); load the lock (128 loc) to $2
BNE $0, $2, LOCK0_T0; check to see if it is locked already ? go back : keep going
ADDI $2, $2, 1; set the lock 
SC $2, 128($0); store the lock back to memory
BEQ $0, $2, LOCK0_T0; if failed, go back to the beginning
...
CACHE 10101, 128($0); otherwise, flush to memory
...
;#################################################################
;------LOADING OPERANDS------
LW $3, 4($0);  loading A0 to $3
LW $4, 68($0); loading B0 to $4
MULT $5, $3, $4; multiply A0 with B0 and put the result in $5
;------END OF LOADING OPERANDS------

;------GETING THE LOCK------
LOCK0_T1: LL $2, 128($0); load the lock (128 loc) to $2
BNE $0, $2, LOCK0_T1; check to see if it is locked already ? go back : keep going
ADDI $2, $2, 1; set the lock 
SC $2, 128($0); store the lock back to memory
BEQ $0, $2, LOCK0_T1; if failed, go back to the beginning
;------END OF GETING THE LOCK------
...
;#################################################################
;------LOADING OPERANDS------
LW $3, 8($0);  loading A0 to $3
LW $4, 72($0); loading B0 to $4
MULT $5, $3, $4; multiply A0 with B0 and put the result in $5
;------END OF LOADING OPERANDS------

;------GETING THE LOCK------
LOCK0_T2: LL $2, 128($0); load the lock (128 loc) to $2
BNE $0, $2, LOCK0_T2; check to see if it is locked already ? go back : keep going
ADDI $2, $2, 1; set the lock 
SC $2, 128($0); store the lock back to memory
BEQ $0, $2, LOCK0_T2; if failed, go back to the beginning
;------END OF GETING THE LOCK------
...
;#################################################################
;------LOADING OPERANDS------
LW $3, 12($0);  loading A0 to $3
LW $4, 76($0); loading B0 to $4
MULT $5, $3, $4; multiply A0 with B0 and put the result in $5
;------END OF LOADING OPERANDS------

;------GETING THE LOCK------
LOCK0_T3: LL $2, 128($0); load the lock (128 loc) to $2
BNE $0, $2, LOCK0_T3; check to see if it is locked already ? go back : keep going
ADDI $2, $2, 1; set the lock 
SC $2, 128($0); store the lock back to memory
BEQ $0, $2, LOCK0_T3; if failed, go back to the beginning
;------END OF GETING THE LOCK------
